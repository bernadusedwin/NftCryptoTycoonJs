            ...
            <div class="rounded-t mb-0 px-4 py-3 border-0">
              ...
              <div class="flex flex-wrap items-center">
                ...
                >
                  ...
                  <div>Item List (MATIC)</div>
                  <div v-if="is_master_network" class="text-red-600">switch to eth network to see result</div>
                  <br/>
                  ...
                  <div>Departure</div>
                  <div class="text-red-600">{{receive_progres_msg}}</div>
                  <br/>
...
<!--                          is_check: {{ item.is_checked }}-->
                          {{infoCheckItem(item.is_checked)}}
                        </td>
                        <td class="border border-amber-600 ...">

                          <button
                              class="text-white p-2 bg-red-400"
                              v-on:click="inquiryHash(item)"
                          >
                            Check
                          </button>

                          <button
                              class="text-white p-2 bg-red-400 m-1"
                              v-on:click="receiveItem(item)"
                          >
                            Receive
                          </button>
...
import new_matic_lib from "../library/new_matic_lib";
...
export default defineComponent({
  ...
  methods: {
    ...
    async bridgeItem(input: NftInquiryItem) {
      // alert("minting")
      await new_trancode_bridge.startBridgeOnMatic(this.current_address, input.token_id);
      await this.reloadData();
    },
    async inquiryHash(input: NftTxInfo) {
      let hash = input.transactionHash;
      let isCheckPoint = await new_matic_lib.checkPoint(hash);
      console.log("hash", hash)

      let msg = "is_check_point: " + isCheckPoint;
      alert(msg)
      if (isCheckPoint) {
        input.is_checked = 1;
      }
    ...
    async receiveItem(input: NftTxInfo) {
      if (this.is_master_network == false){
        alert("must use network eth")
        return;
      }
      this.receive_progres_msg = "loading data ... ";
      try {
        const data = await new_trancode_bridge.inquiryDataBridgeFromTrx(input.transactionHash);
        const content = data.msg;
        this.receive_progres_msg = "open metamask  ... ";
        await new_trancode_bridge.endBridgeOnEth(this.current_address,content);
        this.receive_progres_msg = "";
        this.reloadData();
      }
      catch(e){
        this.receive_progres_msg = "error receive item";
      }
    },
    ...
    async reloadData() {
      ...
      let items2 = await new_trancode_bridge_inquiry.getLogsDepartureMatic(this.current_address)
      console.log("items2", items2)
      this.items_departure = items2;

      let items3 = await new_trancode_bridge_inquiry.getLogsArrivalOnEth(this.current_address)
      console.log("items3", items3)
      this.items_arrival = items3;

      for (const item of this.items_departure) {

        if (this.isAlreadyLanding(item.guid)) {
          item.is_checked = 2;
        } else {
          let isCheckPoint = await new_matic_lib.checkPoint(item.transactionHash);
          if (isCheckPoint) {
            item.is_checked = 1;
          } else {
            item.is_checked = 0;
    ...
    isAlreadyLanding(guid: string) {
      let items = this.items_arrival.filter(x => x.guid == guid);
      let hasData = items.length > 0;
      return hasData;
    },
    infoCheckItem(input: number) {
      if (input == -1) {
        return "inquiry ...."
      } else if (input == 0) {
        return "on progress"
      } else if (input == 1) {
        return "ready for receive cargo"
      } else if (input == 2) {
